...
def fine_tune_model(model_instance, hyperparams, datasets, history, log_dir, training_tag):
    print("Fine tuning")
    model_instance.base_model.trainable = True  # Setting base model to be trainable again
    fine_tune_from = hyperparams["fine_tune_from"]  # Fine-tune from this layer onwards
    for layer in model_instance.base_model.layers[:fine_tune_from]:  # Freeze all the layers before the `fine_tune_from` layer
    ...
    history_fine_tune = model_instance().fit(
        x=datasets['train_ds'],
        epochs=total_epochs,
